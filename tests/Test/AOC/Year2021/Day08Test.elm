module Test.AOC.Year2021.Day08Test exposing (suite)

import AOC.Year2021.Day08 exposing (part1, part2)
import Expect
import List.Extra
...
suite : Test
suite =
    describe "AOC 2021 - Day 08"
        [ test "part 1 sample" <|
            \_ ->
                Expect.equal "26" (part1 (sample |> toList))
        , test "part 2 sample" <|
            \_ ->
                Expect.equal "61229" (part2 (sample |> toList))
        , test "part 1" <|
            \_ ->
                Expect.equal "321" (part1 (input |> toList))
        , test "part 2" <|
            \_ ->
                Expect.equal "1028926" (part2 (input |> toList))
        ]
